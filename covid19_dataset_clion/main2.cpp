...
//    bin_gen();
    for (age_own = 1; age_own < 4 + 1; ++age_own) {
//        bin_gen();
        for (hel_cod = 1; hel_cod < 4 + 1; ++hel_cod) {
//            bin_gen();
            for (job_own = 1; job_own < 3 + 1; ++job_own) {
//                bin_gen();
                for (atc_idx = 1; atc_idx < 5 + 1; ++atc_idx) {
//                    bin_gen();
                    for (sto_ckk = 1; sto_ckk < 2 + 1; ++sto_ckk) {
                        bin_gen();
                    }
                }
            }
